...
Allverb, a cross-platform mobile app built with Expo (SDK 57), React Native, and
expo-router (file-based routing under `src/app`). It targets iOS, Android, and
the web from one TypeScript codebase.

> TODO: replace this with the problem Allverb solves and who it is for once
...
Expo project managed with npm.

- Dev server: `npm run start` (`expo start`; press `w` for web, `i` for iOS simulator, `a` for Android emulator)
- Web: `npm run web`
- iOS: `npm run ios`
...
There is no `build` script. Use `npx expo export` when a production bundle (for
example `npx expo export --platform web`) is needed.

No test runner is configured yet, so there is no test gate. Testing is opt-in;
